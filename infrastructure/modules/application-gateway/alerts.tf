resource "azurerm_monitor_scheduled_query_rules_alert_v2" "ag_certificate_near_expiry" {
  count = var.enable_alerting == true ? 1 : 0

  name                = "${azurerm_application_gateway.this.name}-certificate-near-expiry"
...
QUERY
  ...
  description = "The Application Gateway certificate is nearing expiration."
